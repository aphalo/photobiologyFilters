...
\title{\PBFil Version 0.1.9\\ Catalogue of filters}
...
filter.plotter <- function(filter_name, w.low=280, w.high=1100, 
  ...
  spectrum.data <- data.frame(w.length=seq(w.low, w.high, length.out=300))
spectrum.data$transmittance <- calc_filter_multipliers(spectrum.data$w.length, filter_name, pc.out=FALSE)
fig_linear <- ggplot(aes(x=w.length, y=transmittance), data=spectrum.data) +
  labs(x="Wavelength (nm)", y=ylab, title=filter_name) + ylim(0,1) +
  geom_line()
...
<<plot-schott6>>=
for (filter in c("rg610", "rg630", "rg645", "rg665", "rg695", "rg715", "rg780", "rg830", "rg850", "rg9")) {
  filter.plotter(filter)
}
@
