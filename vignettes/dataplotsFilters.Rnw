...
<<plot-clear>>=
plot(clear.spct, annotations = "colour.guide")
@

Perfectly apaque filter.
<<plot-opaque>>=
plot(clear.spct * 0, annotations = "colour.guide")
@

Flat neutral filters as fractional transmittance.
<<plot-neutral-T>>=
plot(clear.spct * 0.5, annotations = "colour.guide")
@

As absorbance.
<<plot-neutral-A>>=
plot(clear.spct * A2T(1), annotations = "colour.guide")
@
...
for (s in names(acetate.mspct)) {
  print(plot(acetate.mspct[[s]], annotations = "colour.guide") +
          labs(title = paste("Courtaulds cellulose acetate ", 
                             gsub(".", " ", s, fixed = TRUE), 
                             ' acetate.mspct[["', s, '"]]', 
                             sep = "")))
}
...
for (s in names(rosco.mspct)) {
  print(plot(rosco.mspct[[s]], annotations = "colour.guide") +
          labs(title = paste("Rosco ", gsub(".", " ", s, fixed = TRUE), 
                             ' rosco.mspct[["', s, '"]]', 
                             sep = "")))
}
@
