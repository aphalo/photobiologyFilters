...
filter.plotter <- function(filter_name, w.low=280, w.high=1100, 
                           ylab="Spectral transmittance (0 to 1)"){
fig_linear <- 
  ggplot(aes(x=w.length, y=Tfr), data=get(paste(filter_name, ".spct", sep=""))) +
  labs(x="Wavelength (nm)", y=ylab, title=filter_name) + ylim(0,1) +
  geom_line()
# fig_log <- fig_linear + scale_y_log10(limits=c(1e-5,30))
print(fig_linear + 
      stat_peaks(span=71, ignore_threshold=0.25, colour="red", hjust=0, angle=90) + 
