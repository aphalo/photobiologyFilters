...
If we would like to use a different set of wavelengths, then we can build a new filter.spct from scratch.

<<example-1>>=
wl <- 350:450
my.uv.226.spct <- 
  filter.spct(w.length = wl, Tfr = calc_filter_multipliers(wl, "uv.226.new"))
plot(my.uv.226.spct)
@
