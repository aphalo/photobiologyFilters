...
facilitate the calculation of many different quantities that can be derived from spectral irradiance data. The core package is called \PB, and the package described here adds transmittance data for some frequently used filters.

\section{Calculating average transmittance}

<<>>=
uv.226.new.spct <- rosco.mspct$UV_filter_EColour226
...
<<>>=
filters.spct <- rbindspct(list(PE = etola.mspct$Clear_LD_PE_50um,
                               PET = mcdermit.mspct$Autostat_CT5_125um,
                               'Rosco #226' = rosco.mspct$UV_filter_EColour226,
                               'Rosco #312' = rosco.mspct$Canary_Supergel312),
                          idfactor = "filter")
@

<<fig.height=8>>=
plot(filters.spct,
     annotations = c("colour.guide", "boxes", "labels")) +
  facet_grid(filter~.)
@
...
<<example-trim>>=
my.uv.226.spct <-
  trim_spct(uv.226.new.spct, range = c(350,450), byref = FALSE)
plot(my.uv.226.spct)
@
...
<<>>=
plot(sun.spct * rosco.mspct$Canary_Supergel312)
@
...
<<echo=TRUE>>=
all.spct <-
  rbindspct(list('Rosco #312' = sun.spct * rosco.mspct$Canary_Supergel312,
                 'Rosco #226' = sun.spct * rosco.mspct$UV_filter_EColour226,
                 PET = sun.spct * mcdermit.mspct$Autostat_CT5_125um),
            idfactor = "filter")
@
