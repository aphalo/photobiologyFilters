...
The spectral transmittance or absorbance data have been acquired with an assortment of different
...
In addition to these data for filters, the package contains spectral reflectance data for both metal surfaces and various man-made and natural surfaces. All reflactance are for normally incident radiation and total, that is including both especular reflectance and scattering. 

These collections are smaller, so we list the names of all members but only the
first 20 names for filters.
...
To obtain metadata in a tabular for, we can use function `spct_metadata()`.
