...
Individual spectra are stored in classes are derived from tibbles, which in turn
...
For absorptive filters with no coatings reflectance was
...
inevitable artefact of spectral measurements, which has little effect on the
...
In addition to these data for filters, the package contains spectral reflectance data for both metal surfaces and various man-made and natural surfaces. All spectral reflectance data in the package are for normally incident radiation and _total_ values, that is including both specular reflection and scattering. 
...
example is run only if a supported version is loaded.) To limit the number of
...
When metadata are available and the mode of attenuation is `"absorption"` it is
...
To graphically compare filter thicknesses we can pass a collection of spectral objects.
...
Here we use waveband definitions from package 'photobiologyWavebands' and 
