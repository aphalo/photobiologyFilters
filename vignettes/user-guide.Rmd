...
Spectral data are stored in objects of classes defined in package 
'photobiology'. These clases are derived from tibbles, which in turn is
derived from data frames. Consequently the data can be used with native R
functions and operators and with other R packages. 
...
Package 'photobiologyInOut' provides functions that facilitate the exchange of
spectral data with R packages 'colorSpec', 'hyperSpec' and 'pavo'.

The individual spectra are stored in collections of spectra which are objects
derived from R's list class.
...
```
Functions and operators in the packages that are part of the R for photobiology
suite recognize these classes, which allows automation of some computations and
validation of the data values returned by operations or stored.
...
ionic glass and coloured plastics, reflection is not selective, and amounts to
...
these metadata was available an approximate reflectance for normal incidence angle,
...
computed spectral reflectance for some filters or claer glass to serve as 
examples).
...
optical glass. Manufacturers occasionaly may update the specifications
for filters while mantaining the product code.
...
example is run only if a supported version is loaded.)
